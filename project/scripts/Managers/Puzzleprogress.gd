...
var robot_repair_solved : bool = false
var vines_puzzle_solved : bool = false
...
func _ready() -> void:
	...
	robot_repair_solved = puzzles.get("robot_repair", false)
	vines_puzzle_solved = puzzles.get("vines_puzzle", false)

	puzzles_solved = 0
	if pipe_puzzle_solved: puzzles_solved += 1
	if cable_puzzle_solved: puzzles_solved += 1
	if levers_puzzle_solved: puzzles_solved += 1
	if robot_repair_solved: puzzles_solved += 1
	if vines_puzzle_solved: puzzles_solved += 1

	print("PuzzleProgress loaded - solved: ", puzzles_solved)

func puzzle_solved(puzzle_id: String, metadata: Dictionary = {}) -> void:
	match puzzle_id:
		"pipe_puzzle":
			if not pipe_puzzle_solved:
				...
				puzzles_solved += 1
				_save(puzzle_id)
				Analytics.puzzle_solved(puzzle_id, metadata)
		"cable_puzzle":
		...
		"levers_puzzle":
			if not levers_puzzle_solved:
				...
				Analytics.puzzle_solved(puzzle_id, metadata)
		"robot_repair":
			if not robot_repair_solved:
				robot_repair_solved = true
				puzzles_solved += 1
				_save(puzzle_id)
				Analytics.puzzle_solved(puzzle_id, metadata)
		"vines_puzzle":
			if not vines_puzzle_solved:
				vines_puzzle_solved = true
				puzzles_solved += 1
				_save(puzzle_id)
				Analytics.puzzle_solved(puzzle_id, metadata)
